...
class Interface {
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  getInstall() {
    const constructorOverloads = Overloads.getEffectiveOverloads("constructor", this.name, 0, this);

    let constructorBody;
    let argNames = [];
    if (constructorOverloads.length) {
      let minConstructor = constructorOverloads[0];
      for (let i = 1; i < constructorOverloads.length; ++i) {
        if (constructorOverloads[i].nameList.length < minConstructor.nameList.length) {
          minConstructor = constructorOverloads[i];
        }
      }
      ...
      constructorBody = `
        ${conversions.body}
        return iface.setup(Object.create(new.target.prototype), ${passArgs}, privateData);
      `;
    } else {
      constructorBody = `
    ...
    // Copy all the own properties over from the base constructor. This allows to pick up the static properties
    // (WebIDL const attributes).
    this.str += `install(globalObject, privateData) {
      const ${this.name} = function(${argNames.join(", ")}) {
        ${constructorBody}
      }
      Object.defineProperties(${this.name}, Object.getOwnPropertyDescriptors(iface.interface));

      Object.defineProperty(globalObject, "${this.name}", {
        configurable: true,
        overridable: true,
        value: ${this.name}
      });
    ...
    const namedConstructors = this.idl.extAttrs.filter(extAttr => extAttr.name === "NamedConstructor");
    for (const namedConstructor of namedConstructors) {
      const name = namedConstructor.rhs.value;
      const args = namedConstructor.arguments;

      // TODO: Do something with named constructor.
      // How should it invoke the underlying constructor. It is not the case today by an interface may have multiple
      // NamedConstructor. How do we known which named constructor was used?
    }
