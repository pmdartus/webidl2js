...
function isOperation(idl) {
  return idl.type === "operation";
}

function isGetter(idl) {
  return isOperation(idl) && idl.special === "getter";
}

function isSetter(idl) {
  return isOperation(idl) && idl.special === "setter";
}

function isDeleter(idl) {
  return isOperation(idl) && idl.special === "deleter";
}
...
class Interface {
  constructor(ctx, idl, opts) {
    ...
    if (global && (!global.rhs && !global.arguments.length)) {
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  generateIface() {
    ...
    if (shouldExposeRoot) {
      ...
      console.log(JSON.stringify(exposedAttrs, null, 4))
      if (exposedAttrs.length !== 0) {
        const { value } = exposedAttrs[0].rhs;
        if (typeof value === "string") {
          exposedOn = [value];
        } else {
          exposedOn = value.map(token => token.value);
        }
      }

      console.log(exposedOn)
